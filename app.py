...
def ai_game_answers(topic, style):
    """Create two distinct AI answers using the original wit-first logic."""
    style_instruction = player_style_instruction(style)
    topic_instruction = player_topic_instruction(topic)
    logic_mode = style_logic_mode(style, style_instruction)
    ...
    prompt = f"""
あなたは、5〜6歳の子どもと父母が遊ぶカードゲーム「言いカエル」のAIプレイヤーです。
AIカエルは2匹います。名前は付けません。
2匹がそれぞれ1つずつ、合計2つの回答を出してください。
...
- 面白さは、意味不明な奇抜さではなく「意外なのに、言われると筋が通っている」ことから作る。
- 言い方カードは、語尾・擬音・派手な言葉を足すためのカードではなく、「どう見せ直すか」を決める条件として使う。
- 5〜6歳にも場面が浮かぶ具体的な言葉を使うが、幼児向けの単純なギャグへ寄せすぎない。
- 子どもが少し笑えて、大人が聞いても「うまい」と感じる答えを目指す。

【お題の扱い方】
- 必ず「このお題の一般的な意味・定番イメージ」を出発点にする。
- 珍しい例外や、一般的な意味と逆の前提から始めない。
- ひねるのは「評価・見方・役割・たとえ」であって、中心的な事実関係そのものではない。
- お題に複数の定番イメージがあるときは、そのうち1つを明確に選んで使う。

【内部での作り方】
最終回答を出す前に、途中案は出力せず内部で次の順番で考える。
1. お題について、子どもでも分かる普通の特徴を2〜4個考える。
2. それぞれについて、次の発想方向から複数案を作る。
   - 別の物・動物・乗り物・食べ物・自然・身近な場面への「たとえ」
   - あだ名・役割・肩書きのように見せる「呼び名」
   - 欠点→長所、困りごと→役割、普通→特別のような「逆転」
3. その発想を言い方カード「{style}」に合う形へ整える。
4. 少なくとも10案を比較し、「カード適合」「ウィット」「分かりやすさ」「短さ」「意外性」が高いものを選ぶ。
5. 最終的に、発想方向ができるだけ異なる2案を選ぶ。

【禁止】
- ただの擬音、ただの語尾変更、ただの派手語、ただの称号。
- 「最強」「伝説」「ヒーロー」「王様」「達人」などを理由なく付けるだけの答え。
- お題カードの文言「{topic}」を answer にそのまま使うこと。
- 2匹がほぼ同じ発想・同じ比喩を使うこと。
- 日本語以外の文字体系や外国語表記を混ぜること。外来語は一般的なカタカナ表記にする。

【言い方カードへの寄せ方】
- 「口調・形式」を求めるカードは形式を守る。ただし形式だけで終わらず内容にもひねりを入れる。
- 「雰囲気・印象」を求めるカードは、形容詞を足すのではなく、お題の見え方そのものをその印象へ変える。
- 「何かに例える」カードは、対象名を出すだけでなく、お題との共通点をはっきりさせる。
- 「前向き・やさしい・鋭い」など評価方向を求めるカードは、評価語を直接足さず、そう感じる理由がフレーズの中に見えるようにする。
- 「詩的に」は、お題の具体的特徴を1つ固定し、比喩先との共通点が1文で説明できる場合だけ使う。きれいな単語から逆算しない。
- 「皮肉たっぷりに」は、ただの悪口ではなく、表ではほめているように聞こえ、裏では矛盾・やりすぎ・困る点を短く刺す。最後にフォローして丸めない。

【最終チェック】
2回答それぞれについて、以下を満たさなければ内部で作り直す。
- 言い方カードを外しても成立する単なる飾り言葉になっていないか。
- 「なぜこの言葉？」に1文で筋の通った説明ができるか。
- 5〜6歳が場面を思い浮かべられるか。
- 大人が聞いても「少しうまい」と感じるひねりがあるか。
- お題の一般的な意味からずれていないか。
...
【出力ルール】
- answer は説明文にせず、ワンフレーズ、最大でも短いツーフレーズ。30文字以内を目安にする。
- why は子ども向けに1文で、「どこをどう見方を変えたか」が分かるようにする。
- 難解な熟語、抽象語、ネットスラングは避ける。
- 人格・容姿を攻撃しない。人を笑うのではなく、見方のずらしで笑いを作る。
- 2匹に名前は付けない。
""".strip()
    ...
    for attempt in range(4):
        ...
        if attempt:
            retry = (
                "\n\n【作り直し】\n"
                + "、".join(last_problems)
                + "の条件に違反しました。子ども向けの単純なギャグに寄せず、"
                "一休さん的な見方のずらしと言い方カードへの忠実さを優先し、"
                "2匹の発想方向をはっきり分けて作り直してください。"
            )
            if logic_mode == "sarcasm":
                retry += (
                    " 今回は皮肉なので、表ではほめているように聞こえ、"
                    "裏では矛盾・やりすぎ・困る点を短く刺し、最後にフォローせずキレを残してください。"
                )
            elif logic_mode == "poetic":
                retry += (
                    " 今回は詩的表現なので、お題の具体的特徴を1つ固定し、"
                    "比喩先との共通点が子どもにも1文で説明できるものだけにしてください。"
                )

        result = ask_json(prompt + retry, "ai_game_answers_two", schema, max_output_tokens=650)
        problems = []
